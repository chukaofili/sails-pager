...
module.exports = {
    paginate: function(model, criteria, currentPage, perPage, populateData, sort, cb) {
        return new Promise(function(resolve, reject) {
            ...
            var populateData = populateData || [];
            model.count(conditions).then(function(count) {
                var findQuery = model.find(conditions);
                if (sort) {
                    findQuery = findQuery.sort(sort);
                }
                if (_.isArray(populateData) && !_.isEmpty(populateData)) {
                    _(populateData).forEach(function(populate) {
    ...
    paginatePopulate: function(model, criteria, currentPage, perPage, populateData, populate_sort, cb) {
        return new Promise(function(resolve, reject) {
            var pagination = {
                page: parseInt(currentPage) || 1,
                limit: parseInt(perPage) || 20
